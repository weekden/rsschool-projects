...
import type { GarageModel } from '../model';
import type { CreateCarParameters } from '../../../types';
...
export class GarageView {
  ...
  constructor(private model: GarageModel) {
    ...
    this.selectButton = createButton({ text: 'SELECT', classes: ['btn', 'btn-select'] });
    this.removeButton = createButton({ text: 'REMOVE', classes: ['btn', 'btn-remove'] });

    this.startButton = createButton({ text: 'A', classes: ['btn', 'btn-start'] });
    this.stopButton = createButton({ text: 'B', classes: ['btn', 'btn-stop'] });
    this.renderCars();
  }
  ...
  private createItem(car: CreateCarParameters): HTMLElement {
    ...
    const controls = createElement({
      ...
      children: [this.selectButton, this.removeButton, modelLabel],
    });

    const actionButtons = createElement({
      tag: 'div',
      classes: ['garage-item__actions'],
      children: [this.startButton, this.stopButton],
    });
  ...
  private renderCars(): void {
    ...
    if (!cars) return;
    cars.forEach((car) => {
      const carItem = this.createItem({ name: car.name, color: car.color });
